...
class Attack:
    ...
    def search(self):
        ...
        used_time = "Time used to find a distinguisher: {:0.02f} seconds".format(elapsed_time)
        solver_status = f"Solver status: {self.result.status}"
        print(used_time)
        print(solver_status)
        if minizinc.Status.has_solution(self.result.status) or self.result.status == minizinc.Status.ERROR:
            ...
            self.attack_summary += used_time + "\n"
            self.attack_summary += solver_status + "\n"
...
def main():
    ...
    parser = ArgumentParser(description="This tool finds the zero-correlation distinguisher for TWINKLE permutation.\n",
