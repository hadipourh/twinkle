...
class Attack:
    ...
    def search(self):
        ...
        self.result = self.cp_inst.solve(timeout=time_limit, 
                                         ...
                                         optimisation_level=2) # 0: Disable optimisation
    ...
    def parse_solution(self):
        ...
        upper_trail = {"x": [[[[0 for _ in range(80)] for _ in range(4)] for _ in range(4)] for _ in range(self.RU + self.RM + 1)],
                       "y": [[[[0 for _ in range(80)] for _ in range(4)] for _ in range(4)] for _ in range(self.RU + self.RM)],
                       "z": [[[[0 for _ in range(80)] for _ in range(4)] for _ in range(4)] for _ in range(self.RU + self.RM)],
                       "w": [[[[0 for _ in range(80)] for _ in range(4)] for _ in range(4)] for _ in range(self.RU + self.RM)]}
        ...
        for r in range(self.RM + 1):
            for z in range(80):
                for y in range(4):
                    for x in range(4):         
                        upper_trail["x"][self.RU + r][x][y][z] = self.result["xmu"][r][x][y][z]
                        if r < self.RM:
                            upper_trail["y"][self.RU + r][x][y][z] = self.result["ymu"][r][x][y][z]
                            upper_trail["z"][self.RU + r][x][y][z] = self.result["zmu"][r][x][y][z]
                            upper_trail["w"][self.RU + r][x][y][z] = self.result["wmu"][r][x][y][z]
        lower_trail = {"x": [[[[0 for _ in range(80)] for _ in range(4)] for _ in range(4)] for _ in range(self.RM + self.RL + 1)],
                       "y": [[[[0 for _ in range(80)] for _ in range(4)] for _ in range(4)] for _ in range(self.RM + self.RL)],
                       "z": [[[[0 for _ in range(80)] for _ in range(4)] for _ in range(4)] for _ in range(self.RM + self.RL)],
                       "w": [[[[0 for _ in range(80)] for _ in range(4)] for _ in range(4)] for _ in range(self.RM + self.RL)]}
        for r in range(self.RM + 1):
            for z in range(80):
                for y in range(4):
                    for x in range(4):         
                        lower_trail["x"][r][x][y][z] = self.result["xml"][r][x][y][z]
                        if r < self.RM:
                            lower_trail["y"][r][x][y][z] = self.result["yml"][r][x][y][z]
                            lower_trail["z"][r][x][y][z] = self.result["zml"][r][x][y][z]
                            lower_trail["w"][r][x][y][z] = self.result["wml"][r][x][y][z]
        for r in range(self.RL + 1):
            for z in range(80):
                for y in range(4):
                    for x in range(4):         
                        lower_trail["x"][self.RM + r][x][y][z] = self.result["xl"][r][x][y][z]
                        if r < self.RL:
                            lower_trail["y"][self.RM + r][x][y][z] = self.result["yl"][r][x][y][z]
                            lower_trail["z"][self.RM + r][x][y][z] = self.result["zl"][r][x][y][z]
                            lower_trail["w"][self.RM + r][x][y][z] = self.result["wl"][r][x][y][z]
        input_diff = ""
        for y in range(4):
            for x in range(4):
        ...
        for y in range(4):
            for x in range(4):
                input_diff_middle += "".join(list(map(str, self.result["xmu"][0][x][y]))).replace("-1", "?") + "\n"
        output_mask_middle = ""
        for y in range(4):
            for x in range(4):
                output_mask_middle += "".join(list(map(str, self.result["xml"][self.RM][x][y]))).replace("-1", "?") + "\n"                
        output_mask = ""
        for y in range(4):
            for x in range(4):
                output_mask += "".join(list(map(str, self.result["xl"][self.RL][x][y]))).replace("-1", "?") + "\n"
        
        revised_pu = 0
        for r in range(self.RU):
            for z in range(80):
                for y in range(4):
                    revised_pu += 3*self.result["pu"][r][0][y][z] + 2*self.result["pu"][r][1][y][z] +  1.415*self.result["pu"][r][2][y][z] 
        attack_summary = f"Attack summary:\n"
        attack_summary += f"Solver status: {self.result.status}\n"
        attack_summary += f"Setting:\nRU: {self.RU}, RM: {self.RM}, RL: {self.RL}\n"
        attack_summary += f"WU: {self.WU}, WM: {self.WM}, WL: {self.WL}\n"
        attack_summary += "#"*50 + "\n"
        attack_summary += f"input:  \n{input_diff}"
        attack_summary += "#"*50 + "\n"
        attack_summary += f"input difference (for middle part):  \n{input_diff_middle}"
        attack_summary += "#"*50 + "\n"
        attack_summary += f"output mask (for middle part):  \n{output_mask_middle}"
        attack_summary += "#"*50 + "\n"
        attack_summary += f"output: \n{output_mask}"
        attack_summary += "#"*50 + "\n"
        attack_summary += f"PU = {revised_pu:.3f}\n"
        attack_summary += f"CM = {self.result['CM'] / 10:.3f}\n"
        attack_summary += f"CL = {self.result['CL'] / 10:.3f}\n"
...
def main():
    ...
    parser = ArgumentParser(description="This tool finds the differential-linear distinguisher for TWINKLE permutation.\n",
                            formatter_class=RawTextHelpFormatter)
    
    parser.add_argument("-RU", type=int, default=1, help="Number of rounds in EU")
    parser.add_argument("-RM", type=int, default=3, help="Number of rounds in EM")
    parser.add_argument("-RL", type=int, default=0, help="Number of rounds in EL")
    parser.add_argument("-WU", type=int, default=1, help="Number of rounds in EU")
    parser.add_argument("-WM", type=int, default=1, help="Number of rounds in EM")
    parser.add_argument("-WL", type=int, default=1, help="Number of rounds in EL")
